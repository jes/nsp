...
void usage(void) {
  fprintf(stderr,
          ...
          "to send all commands to one node, then all commands to the next, "
          "and so on. Toggling this will cause nsp to send the first command "
          "to each node, and then the second command to each node, etc.\n"
          "The output will be printed in the order the commands were sent.\n"
          "\n"
          "Node files should contain lists of hostnames or IP addresses, "
          "one per line, optionally with port numbers, like so:\n"
          "  router3:5000\n"
          "for port 5000 on node router3. If no port number is given, 198 is "
          "assumed.\n"
          "\n"
